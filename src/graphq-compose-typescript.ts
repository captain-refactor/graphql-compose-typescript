...
import {
  InputFieldCreator,
  OutputFieldCreator
} from "./type-composer-creation/field-creators";
...
import { WrapComposerSpecKeeper } from "./type-composer-creation/wrap-composer-spec-keeper";

export type ProvidenOutputType =
...
export class ArrayTypeNotSpecified extends Error {
  constructor(constructor: Function, propertyName: string) {
    super(
      `${
        constructor.name
      }.${propertyName} does not have specified array item type`
    );
  }
}
...
export class ClassSpecialist {
  isClassType(
    type: ProvidenOutputType | ProvidenInputType
  ): type is ClassType<any> {
...
export class GraphqlComposeTypescript {
  constructor(
    public readonly schemaComposer: SchemaComposer<any>,
  ...
  getComposer<T>(
    typeOrInstance: ClassType<T>
  ): ObjectTypeComposer<T, DefaultContext<T>> {
  ...
  static create(schemaComposer: SchemaComposer<any>) {
    ...
    const inputComposerCreator = new InputComposerCreator(
      schemaComposer,
      typeNameKeeper,
      classSpecialist
    );
    const outputComposerCreator = new OutputComposerCreator(
      schemaComposer,
      typeNameKeeper,
      classSpecialist
    );
    const inputTypeComposerBaseQueue = new BaseQueue<InputTypeComposer>(
      inputComposerCreator,
      InputTypeQueueItem
    );
    const typeComposerBaseQueue = new BaseQueue<ObjectTypeComposer>(
      outputComposerCreator,
      OutputTypeQueueItem
    );
    const queue = new Queue(inputTypeComposerBaseQueue, typeComposerBaseQueue);
    const providenInputTypeConvertor = new ProvidenTypeConvertor<
      ComposeInputType
    >(
      classSpecialist,
      fieldSpecKeeper,
    ...
    const providenOutputTypeConvertor = new ProvidenTypeConvertor<
      ComposeOutputType<any, any>
    >(
      classSpecialist,
      fieldSpecKeeper,
    ...
    const argumentTypeConvertor = new ArgumentTypeConvertor(
      ptk,
      providenInputTypeConvertor
    );
    ...
    const propertyOutputTypeConvertor = new PropertyTypeConvertor<
      ComposeOutputType<any, any>
    >(providenOutputTypeConvertor, ptk);
    ...
    const outputFieldCreator = new OutputFieldCreator(
      ...
      const wrapComposerSpecKeeper = new WrapComposerSpecKeeper();
      const typeComposerComposerBuilder = new ComposerBuilder<ObjectTypeComposer>(
      fieldSpecKeeper,
      schemaComposer,
      outputFieldCreator,
      wrapComposerSpecKeeper
    );
    const propertyInputTypeConvertor = new PropertyTypeConvertor<
      ComposeInputType
    >(providenInputTypeConvertor, ptk);
    const inputFieldCreator = new InputFieldCreator(
      propertyInputTypeConvertor,
      schemaComposer
    );
    const inputTypeComposerComposerBuilder = new ComposerBuilder<
      InputTypeComposer
    >(
      fieldSpecKeeper,
      schemaComposer,
    ...
    const typeComposerCreator = new ComposerCreator(
      outputComposerCreator,
      inputComposerCreator,
      typeComposerComposerBuilder,
      inputTypeComposerComposerBuilder
    );
    const resolverSpecStorage = new ResolverSpecStorage();
    const queueSolver = new QueueSolver(
      queue,
      typeComposerComposerBuilder,
      inputTypeComposerComposerBuilder,
      classSpecialist
    );
    const resolverBuilder = new ResolverBuilder(
      propertyOutputTypeConvertor,
      argumentsBuilder,
      argumentsMapper,
      queueSolver,
