...
export class ContextSpecKeeper {
    ...
    setContextParameter<T>(type: ClassWithContextMap<T>, method: StringKey<T>, index: number) {
        this.setContextMap(type, this.getContextMap(type).set(method, index));
    }
    getContextParameterIndex<T>(type: ClassWithContextMap<T>, method: StringKey<T>) {
